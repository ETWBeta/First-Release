...
<!-- Withdraw swal -->
<?php
$withdr = "<br><a href='?withdraw'><button type='button' class='button2'>Withdraw</button></a>";
$online = " <span style='color:green;'>Online</span><br>";
$offline = " <span style='color:red;'>Offline</span><br>";
?>
<script>
function withdraw() {
  
swal({
  html: "<span class='colored'>Withdraw to wallet</span><p>As of current rate 1000 Points equals to 1 USD</p><br><p>You have <?php echo $points; ?> points which turns out to be <?php echo round($points / 100 / $node->textContent,6) / 10; ?> TRX</p><br><?php $host = '86.38.40.97'; if($socket =@ fsockopen($host, 80, $errno, $errstr, 5)) { echo 'SERVER IS CURRENTLY'; echo $online; echo $withdr; fclose($socket); } else { echo 'SERVER IS CURRENTLY'; echo $offline; } ?>",
  buttonsStyling: false,
  heightAuto: false
})
...
include('dbcon.php');	
		...
		$points = $row['points'];
		$wallet = $row['wallet'];	
        $points2trx = round($points / 100 / $node->textContent,6) / 10;		
		if ($points > 0) {	
		$query = "INSERT INTO transitions (trx, wallet, username) 
  			  VALUES('$points2trx', '$wallet', '$username')"; // Adding user wallet and amount of TRX to database
		file_get_contents('http://86.38.40.97/transactionSender.php?wallet=' . $wallet . '&amount=' . $points2trx); // Sending transaction		  
		mysqli_query($db, $query);
		$query2 = "UPDATE users SET points = points - $points WHERE username='$username'";
		mysqli_query($db, $query2);	
		echo '<script>swal({text: "TRX has been sent to your wallet", buttonsStyling: false, heightAuto: false}).then(function(){ location.reload();});</script>';		
		} 
		else {	
